...
$stmt = $pdo->prepare("
    SELECT iv.*, c.cell_code, c.id 
    FROM production.iv_measurements iv
    JOIN cells c
    ON iv.cell_id = c.id
    WHERE iv.id = ?");
$stmt->execute([$id]);
...
$pts = $pdo->prepare("SELECT voltage_v, current_density_ma_cm2, direction FROM production.iv_data_points WHERE measurement_id = ? ORDER BY voltage_v ASC");
$pts->execute([$id]);
...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3>IV Analysis: <?= htmlspecialchars($iv['cell_code']) ?></h3>
        <a href="iv_test_list.php" class="btn btn-sm btn-light">Back to List</a>
    </div>
    <div class="row mb-4">
        <div class="col-md-4">
