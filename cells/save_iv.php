...
try {
    ...
    // 2. Resolve Cell ID
    $stmt = $pdo->prepare("SELECT id FROM cells WHERE cell_code = ?");
    $stmt->execute([$meta['cell_code']]);
    $cell_id = $stmt->fetchColumn();

    if (!$cell_id) {
        throw new Exception("Cell code " . htmlspecialchars($meta['cell_code']) . " not found in database.");
    }
    ...
    $sql = "INSERT INTO iv_measurements (
                cell_id, test_date, operator, 
    ...
    $stmt_pt = $pdo->prepare("INSERT INTO iv_data_points (measurement_id, voltage_v, current_density_ma_cm2, direction) VALUES (?, ?, ?, ?)");
    ...
    header("Location: iv_test_list.php?success=1");
    exit;
