...
<script>
const statusData = {
    labels: <?= json_encode(array_keys($batchStatusData)) ?>,
    datasets: [{
        label: 'Batches',
        data: <?= json_encode(array_values($batchStatusData)) ?>,
...
const yieldData = {
    labels: <?= json_encode(array_column($yieldData,'batch_code')) ?>,
    datasets: [{
        label: 'Yield %',
        data: <?= json_encode(array_column($yieldData,'yield')) ?>,
        backgroundColor: '#0d6efd'
    }]
};
new Chart(document.getElementById('batchYieldChart'), {
    type: 'bar',
    data: yieldData,
